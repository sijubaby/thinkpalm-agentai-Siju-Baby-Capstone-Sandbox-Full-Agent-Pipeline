...
from maritime_qa.api.service import (
    clear_runs,
    delete_run,
    execute_pipeline,
    get_run_for_ui,
...
from maritime_qa.paths import DOCS_DIR, OUT_DIR, RUNS_DIR, SAMPLES_DIR

STATIC = Path(__file__).resolve().parents[1] / "ui" / "static"

SAMPLE_FILES = {
    "crew-cert": SAMPLES_DIR / "description-crew-cert.txt",
    "ais": SAMPLES_DIR / "description-ais.txt",
    "port-workflow": SAMPLES_DIR / "description-port-workflow.txt",
}
...
class MaritimeHTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/v1/openapi":
            return self._serve_file(DOCS_DIR / "openapi.json", "application/json")
    ...
    def do_DELETE(self) -> None:
        ...
        if path == "/api/v1/runs":
            ...
            try:
                ...
                return self._json(500, {"error": str(exc)})
            return self._json(200, {"cleared": removed})
        ...
        if run_match:
            if not _check_api_key(self):
                return self._json(401, {"error": "Invalid or missing X-API-Key"})
            try:
                delete_run(run_match.group(1))
            except FileNotFoundError as exc:
                return self._json(404, {"error": str(exc)})
            except Exception as exc:
                return self._json(500, {"error": str(exc)})
            return self._json(200, {"deleted": run_match.group(1)})
